...
def get_domain_data(domain_name, split_type='train', num_sample=None, cache_dir='./data'):
    ...
    if domain_name == 'medical': 
        dataset = load_dataset(
            ...
            cache_dir=cache_dir, 
            token=os.getenv("HF_TOKEN")  # Sử dụng token từ biến môi trường nếu có
        )
        ...
        return [{'en' : ex['en'], 'vi': ex['vi']} for ex in target_dataset['translation']]
        ...
        if num_sample:
            target_dataset = dataset.select(range(min(num_sample, len(dataset))))
